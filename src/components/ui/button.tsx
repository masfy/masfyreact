...
import { cn } from "../../lib/utils"; // Pastikan path ke utils.ts benar
...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 dark:ring-offset-gray-950 dark:focus-visible:ring-blue-400",
  {
    variants: {
      variant: {
        default: "bg-blue-600 text-white hover:bg-blue-700 dark:bg-blue-500 dark:text-gray-950 dark:hover:bg-blue-600",
        destructive: "bg-red-500 text-white hover:bg-red-600 dark:bg-red-900 dark:text-gray-50 dark:hover:bg-red-800",
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        className={cn(buttonVariants({ variant, size, className }))} // Sangat penting: menerima kelas dari prop
        ref={ref}
        {...props}
      />
    );
